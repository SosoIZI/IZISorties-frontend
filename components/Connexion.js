...
function Connexion({ isConnected = true, showModal, handleClose }) { 
  ...
  const handleSubmit = (e) => {
    ...
    e.preventDefault();// empeche comportement par défaut de JS
    handleClose();
    ...
    fetch("http://localhost:3000/users/signin", {
      ...
      .then((data) => {
        ...
        if (data.result) {
          ...
          setSignInPassword("");
          setSignInEmail("");
          router.push("/Home"); // si il ya une réponse, ds le then __> faire le dispatch, réinitialiser les champs et rediriger vers la page home
        } else {
          Swal.fire({
